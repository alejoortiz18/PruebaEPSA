...
namespace EpsaDAL
{
    ...
    public class ConsumoPorTramoDAL
    {
        ...
        {
            List<ObtenerTramosConMayoresPerdidasDto> oht = new List<ObtenerTramosConMayoresPerdidasDto>();
            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(_context))
                {
                    ...
                    using (SqlCommand command = new SqlCommand("ObtenerTramosConMayoresPerdidas", connection))
                    {
                        ...
                        ObtenerTramosConMayoresPerdidasDto historia = new ObtenerTramosConMayoresPerdidasDto();
                        using (SqlDataReader reader = command.ExecuteReader())
                        {
                            while (reader.Read())
                            {
                                historia.Tramo = reader["Tramo"].ToString();
                                historia.Fecha = Convert.ToDateTime(reader["Fecha"]);
                                historia.ResidencialWH = (int)reader["ResidencialWH"];
                                historia.ComercialWH = Convert.ToInt32(reader["ComercialWH"]);
                                historia.IndustrialWH = (int)reader["IndustrialWH"];
                                oht.Add(historia);
                            }
                        }
                    }
